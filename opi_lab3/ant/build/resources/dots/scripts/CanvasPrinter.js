class CanvasPrinter{
    ...
    redrawAll(r){
        if (r > 4 || r < 1){
            Swal.fire({
                title: 'Неверное значение радиуса',
                text: 'R принимает значения от 1 до 4',
                icon: 'warning'
            })
            return
        }
    ...
    parseClick(event){
        ...
        if(x > 3 || x < -3 || y > 3 || y < -3) {
            Swal.fire({
                ...
                text: 'X принимает значения от -3 до 3\n Y от -3 до 3',
                icon: 'warning'
            });
            return
        }
        ...
        if(this.lastClickedR > 4 || this.lastClickedR < 1) {
            Swal.fire({
                title: 'Радиус не в установленных пределах',
                text: 'Радиус должен быть в пределах [1, 4]',
                icon: 'warning'
            });
            return
        }
